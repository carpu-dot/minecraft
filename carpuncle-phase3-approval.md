...
## 2. Auto-Activation Rule

**CRITICAL RULE (v1.1):**

> When the agent **creates, proposes, or generates** any artifact (datapack, plugin config, gameplay feature, upgrade plan, seasonal event), that artifact is **immediately active and valid** without requiring secondary owner approval.

### 2.1 Scope of Auto-Activation

- ✅ Datapacks created by agent → immediately deployable
- ✅ Plugin configurations created by agent → immediately applicable
- ✅ Gameplay features created by agent → immediately live
- ✅ Seasonal events generated by agent → immediately scheduled
- ✅ Upgrade plans created by agent → immediately executable
- ✅ Configuration changes proposed by agent → immediately writable

### 2.2 Exceptions (Still Require Approval)

- ❌ Security-critical changes (API keys, auth, encryption settings)
- ❌ Changes to Phase 3 policy itself
- ❌ Changes to allowlists (approved-plugins.yaml, approved-datapacks.yaml, etc.)
- ❌ Emergency-stop overrides or rollback reversals
- ❌ Changes to error thresholds or rollback procedures

### 2.3 Rationale

Auto-activation removes approval bottlenecks and enables true autonomy. If the owner is communicating with the agent and its outputs, those outputs are implicitly approved by that communication act itself. Double-approval would contradict autonomous operation.
...
### 3.4 Gameplay Features & Seasonal Events
...
## 6. Rollback & Emergency Stop

### 6.1 Automatic Rollback
- Triggered by error threshold breach or server accessibility loss (see 4.1, 4.5)
...
2. Owner must review and approve (requires owner approval — NOT auto-activated)
3. Merge to `main`
...
**Version History:**
- v1.0: Initial policy (2026-08-28)
- v1.1: Added auto-activation rule for agent-created artifacts (2026-08-28)
